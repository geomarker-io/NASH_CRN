...
csd_mua <- d %>%
  filter(
    `MUA/P Status Code` %in% c("P", "D"), # keep only designated and proposed for withdrawal
    `Designation Type` %in% c(
...
mua_tracts <- mua_tracts %>%
  mutate(
    overlaps_csd_mua = ifelse(mua.x == "Yes" | mua.y == "Yes" | mua == "Yes", "Yes", "No"),
    overlaps_csd_mua = ifelse(is.na(overlaps_csd_mua), "No", overlaps_csd_mua)
  ) %>%
  select(tract_id, overlaps_csd_mua)
