...
d_states_abb <- tigris::states() %>%
  sf::st_drop_geometry() %>%
  filter(! NAME %in% c('American Samoa', 'Guam', 'Northern Mariana Islands',
...
cw <- purrr::map(dest, ~read_delim(.x, delim = ",", col_names = cw_col_names,
...
cw <- cw %>%
  select(census_tract_id_2000 = GEOID00,
